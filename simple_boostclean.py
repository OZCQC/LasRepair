...
def load_csv_data(dirty_path, clean_path, max_rows=1000):
    """Load dirty and clean datasets using csv module"""
    ...
    with open(dirty_path, 'r') as f:
        ...
        for i, row in enumerate(reader):
            if i >= max_rows:
                break
            dirty_data.append([str(cell) if cell else "" for cell in row])
    ...
    with open(clean_path, 'r') as f:
        ...
        for i, row in enumerate(reader):
            if i >= max_rows:
                break
            clean_data.append([str(cell) if cell else "" for cell in row])
    
    print("Loaded {} dirty rows and {} clean rows with {} columns".format(
        len(dirty_data), len(clean_data), len(header)))
    
    return dirty_data, clean_data, header

def detect_errors_simple(data):
...
def simple_repair_cells(data, error_cells):
    ...
    for col_idx, error_rows in errors_by_col.items():
        ...
        # Use most common value for repair
...
def calculate_f1_simple(original_data, corrected_data, ground_truth_data):
...
def calculate_error_stats(dirty_data, clean_data):
...
def main():
    ...
    # Load data (using first 500 rows for faster testing)
    try:
        dirty_data, clean_data, column_names = load_csv_data(
            'datasets/shuttle/dirty_20.csv', 
            'datasets/shuttle/clean.csv', 
            max_rows=500
        )
    except Exception as e:
    ...
    # Calculate basic error statistics
    error_count, total_cells, error_rate = calculate_error_stats(dirty_data, clean_data)
    
    # Detect errors using ActiveDetect
    error_cells = detect_errors_simple(dirty_data)
    ...
    if error_cells:
        # Apply simple repair
        corrected_data = simple_repair_cells(dirty_data, error_cells)
        
        # Calculate F1 score
        f1, precision, recall = calculate_f1_simple(dirty_data, corrected_data, clean_data)
