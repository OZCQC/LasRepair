...
def detect_errors(data):
    ...
    try:
        ...
        print(f"Detected {len(error_cells)} potential error cells")
...
def simple_repair(data, error_cells):
    ...
    for col_idx, error_rows in errors_by_col.items():
        print(f"Repairing column {col_idx} with {len(error_rows)} errors")
...
def main():
    ...
    if not error_cells:
        ...
        try:
            ...
            print(f"Detected {len(error_cells)} potential error cells with relaxed threshold")
        except Exception as e:
