...
def create_evaluation_format(clean_csv_path, output_path):
    ...
    for tid, row in df.iterrows():
        for attr in df.columns:
            # Add 'val_' prefix to ensure values are treated as strings
            val = f"val_{row[attr]}" if pd.notna(row[attr]) else "val_"
            eval_data.append({
                'tid': tid,
                'attribute': attr, 
                'correct_val': val
...
create_evaluation_format('../../../datasets/shuttle/clean.csv', 
                        '../../../datasets/shuttle/shuttle_clean_eval.csv')
...
hc.load_data('shuttle', '../../../datasets/shuttle/dirty_20.csv')
hc.load_dcs('../../../datasets/shuttle/shuttle_constraints.txt')
...
print("=== SHUTTLE DATASET EVALUATION RESULTS ===")
report = hc.evaluate(fpath='../../../datasets/shuttle/shuttle_clean_eval.csv',
                    tid_col='tid',
                    attr_col='attribute', 
                    val_col='correct_val')
