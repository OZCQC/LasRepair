...
public class Grounding {
	 ...
	 * @throws IOException 
	 */
	public HashMap<String, Double> constructMRF() throws IOException{
		...
		db.update(sql);
		this.computeActiveClauses(cbuffer);//计算团的权重
		this.addSoftEvidClauses(mln.relAtoms, cbuffer);//添加单节点的权重，即把单个节点也当做一个团来看待
		
		//TODO!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		this.addKeyConstraintClauses(mln.relAtoms, cbuffer);
		
		createClauseTable(mln.relClauses);
		numClauses = this.consolidateClauses(cbuffer, mln.relClauses);//添加数据到clauses表
		...
		ArrayList<MLNClause> clausesLits = getClausesLits();//返回Clauses的权重
		HashMap<String, Double> attributes = updateClausesInfo(clausesLits);
...
(Config.learning_mode && c.isFixedWeight == false && c.hasEmbeddedWeight() ? c.sqlFromList_noModel : c.sqlFromList)
	...
	/**
	 * 从数据集中读取Clause的atomids和clause weight
	 * author gecongcong
	 * */
	private ArrayList<MLNClause> getClausesLits(){
		Array lits = null;
		ArrayList<MLNClause> clauses = new ArrayList<MLNClause>();
		String sql;
		try {
			sql = "SELECT lits, weight FROM markov_schema.mln0_clauses\n" +
					"ORDER BY cid ASC";
			ResultSet rs = db.query(sql);
			while(rs.next()){
				lits = rs.getArray("lits");
				double weight = rs.getDouble("weight");
				MLNClause mlnClause = new MLNClause();
				mlnClause.weight = weight;
				mlnClause.atomids=(Integer[])lits.getArray();
				if(mlnClause.atomids.length>1){
					mlnClause.atomids[0] = Integer.parseInt(mlnClause.atomids[0].toString().replaceAll("-",""));
				}
	...
	/**
	 * 为Clauses添加atomid对应的值
	 * */
	private HashMap<String, Double> updateClausesInfo(ArrayList<MLNClause> clauses){
		String sql;
		ResultSet rs = null;
		HashMap<String, Double> attributes = new HashMap<String, Double>();
		try {
			for(MLNClause c : clauses){
				Integer[] atomids = c.atomids;
				c.values = new ArrayList<String>(atomids.length);
				//System.out.println(Arrays.toString(atomids));
				for(int atomid : atomids){
					for(Type t : mln.nameMapType.values()){
						String relname = t.getRelName().replace("type_","").toLowerCase();
						String attributename = relname.replaceAll("value","");
						sql = "SELECT constantvalue FROM markov_schema.type_"+relname+", markov_schema.pred_"+attributename+"\n" +
								"WHERE markov_schema.pred_"+attributename+".atomid="+atomid+
								" AND markov_schema.pred_"+attributename+"."+relname+"1=markov_schema.type_"+relname+".constantid";
						//System.out.println(sql);
						rs = db.query(sql);
						if(rs.next()){
							String value = rs.getString("constantvalue");
							//System.out.println("atomid="+atomid+" value="+value);
				...
				//System.out.println("c.values.toString()="+Arrays.toString(tmp_value));
			}
			rs.close();
		}catch (SQLException e) {
			e.printStackTrace();
