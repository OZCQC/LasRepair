...
class MultiModelIterativeGenerativeRepair():
    ...
    def run(self):
        ...
        self.last_f1 = 0
        prev_weight_used = None  # for Δω convergence check
        ...
        for iteration in range(self.max_iteration):
            ...
            if self.use_weight:
                ...
                old_weight_used = None if prev_weight_used is None else prev_weight_used.copy()
                # Update the normalised row-level weight vector (ω*) from the current confidence/uncertainty matrix.
                self.compute_weight_used()

                # Δω: L1 distance between consecutive normalised weight vectors.
                if old_weight_used is None:
                    print('Δω (L1): N/A (first iteration)')
                else:
                    delta_omega = float(np.sum(np.abs(self.weight_used - old_weight_used)))
                    change_rate = (delta_omega - old_weight_used) / old_weight_used
                    print(f'change rate: {change_rate:.6f} (eps={self.omega_eps})')
                    if change_rate < self.omega_eps:
                        print('Early stop: change rate < eps')
                        break
                prev_weight_used = self.weight_used.copy()
...
if __name__ == "__main__":
    ...
    # nohup python -u -m EMCL.EMCL.LasRepair --experiment beers --gpu cuda:5 >> /data1/qianc/logs/EMCL/las_beers.txt 2>&1 &
    ...
    args.add_argument("--omega_eps", type=float, default=0.01)
    args.add_argument("--sample_num", type=int, default=0)
    args.add_argument("--temperature", type=float, default=1.0)
    args.add_argument("--correct_prop", type=float, default=0.2)
    ...
    if args.error_rate != 0:
        dirty_path = '/data1/qianc/EMCL/datasets/' + args.experiment + '/' + args.experiment + '_' + str(int(args.error_rate*100)) + '_error.csv'
    ...
    r = a.get_res()
    if args.error_rate == 0:
        path = './result/' + args.experiment + '_repaired_original.csv'
    else: 
        path = './result/' + args.experiment + '_' + str(int(args.error_rate*100)) + '_error_repaired.csv'
